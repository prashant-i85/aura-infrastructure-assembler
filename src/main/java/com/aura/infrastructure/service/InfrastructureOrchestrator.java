...
public class InfrastructureOrchestrator {
    ...
    private final TerraformGeneratorService generatorService;
    ...
    public InfrastructureResponse initiateProvisioning(InfrastructureRequest request) {
        ...
        InfrastructureResponse response = InfrastructureResponse.builder()
                ...
                .message("Infrastructure provisioning request accepted. " +
                         "Poll GET /api/infrastructure/status/" + requestId + " to track progress.")
                .createdAt(now)
                .updatedAt(now)
                .build();
        ...
        // Kick off background execution — returns immediately
        provisionAsync(request, requestId);
     ...
     * Progresses through: IN_PROGRESS → generate → init → plan → apply → output → SUCCESS
    ...
    public void provisionAsync(InfrastructureRequest request, String requestId) {
        ...
        try {
            ...
            workspacePath = generatorService.generateConfiguration(request, requestId);
            ...
            applySuccessfulOutput(requestId, tfOutput);
    ...
    private void applySuccessfulOutput(String requestId, TerraformOutput tfOutput) {
        repository.findById(requestId).ifPresent(r -> {
            ...
            r.setMessage("EC2 instance provisioned successfully.");
            r.setUpdatedAt(LocalDateTime.now());
        });
    }
}
